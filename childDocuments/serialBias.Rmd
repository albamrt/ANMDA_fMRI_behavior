...
if(min_SEM < min_SEM3){
  mod <- lme(error ~ group*time*DoG,  random = ~(1+time)|subject, 
             data = data, na.action = na.omit)
}else{mod <- lme(error ~ group*time*DoG3, random = ~(1+time)|subject, 
                 data = data, na.action = na.omit)
}
anova(mod) %>%
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
        caption = 'Summary of a linear model where the response is the error and regressors are group, time, a derivative of gaussian, the interaction between these three and subject as a random factor.') %>%
