...
### Folded serial bias
```{r warning = FALSE, fig.cap = 'Dashed lines show the mean serial bias. Shading, $+-$ s.e.m. Solid lines show linear model fits (with the non-folded serial bias).'}
window = pi/5
step = pi/20
...
```{r fig.cap = 'Mean folded serial bias by subect. Its mean and error bars are also plotted. The romboid is the predicted value by the mixed model (with the non-folded serial bias).'}
groupedSubjSes %>%
...
if(min_MSE < min_MSE3){
  if (DoG[6] > 0){
    data$previousDoG <- dog1(sigma = sigma, x = data$diffresp)*-1
    }else{
    data$previousDoG <- dog1(sigma = sigma, x = data$diffresp)
    }}else{
      if (DoG3[6] > 0){
      data$previousDoG <- dog3(sigma = sigma, x = data$diffresp)*-1
      }else{
      data$previousDoG <- dog3(sigma = sigma, x = data$diffresp)
      }
    }
