...
```{r fig.cap = 'Mean folded serial bias by subect. Its mean and error bars are also plotted. The romboid is the predicted value by the mixed model.', eval = FALSE}
groupedSubjSes %>%
  ...
  if(simm$min_MSE1[i] < simm$min_MSE3[i]){
    if (simm$DoG1[[i]][6] > 0){
      data[data$group == levels(simm$group)[i],'previousDoG'] <- dog1(sigma = simm$sigma1[[i]],  
                            x = abs(data[data$group == levels(simm$group)[i],]$diffresp))*-1
      }else{
      data[data$group == levels(simm$group)[i],'previousDoG'] <- dog1(sigma = simm$sigma1[[i]], 
                            x = abs(data[data$group == levels(simm$group)[i],]$diffresp))
      }}else{
        if (simm$DoG3[[i]][6] > 0){
...
if(simm$min_MSE1[i] < simm$min_MSE3[i]){
  if (simm$DoG1[[i]][6] >0){
    data[data$group == levels(simm$group)[i],'futureDoG'] <- dog1(sigma = simm$sigma1[[i]],
                        x = abs(data[data$group == levels(simm$group)[i],]$difffuture))*-1
    }else{
    data[data$group == levels(simm$group)[i],'futureDoG'] <- dog1(sigma = simm$sigma1[[i]],
                        x = abs(data[data$group == levels(simm$group)[i],]$difffuture))
    }}else{
      if (simm$DoG3[[i]][6] >0){
