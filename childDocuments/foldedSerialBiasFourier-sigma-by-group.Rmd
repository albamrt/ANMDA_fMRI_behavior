...
data$diffstim <- abs(data$diffstim)
```
...
simfolded <- data %>%
  ...
  mutate(cv_dog1 = map(data, function(df) sapply(Sigma, 
      function(x) replicate(n_reps, crossValSigmaFolded(x, df, derivative = 1)))),
         cv_dog3 = map(data, function(df) sapply(Sigma, 
      function(x) replicate(n_reps, crossValSigmaFolded(x, df, derivative = 3)))))
save(simfolded, file = "data/simFolded-by-group.RData")
```

```{r fig.cap = 'Left: Mean squared error for stratified hyperparameter optimization using cross-validation (1,000 repetitions, training set size = .33 from each subject) for first- (black) and third- (orange) derivative-of-Gaussian fits. Hyperparameters are different values of scale parameter σ of the underlying Gaussian with location hyperparameter μ = 0. Right: Shape of first- and third-derivative-of-Gaussian fits with optimal hyperparameter σ and μ = 0.', out.width='49%', fig.ncol = 2}
load(file = 'data/simFolded-by-group.RData')

diffstims <- seq(0, pi/2, length.out = 30)
...
p1 <- list()
p2 <- list()
ylims <- c(min(c(simm$cv_dog1[[1]], 
                 simm$cv_dog1[[2]], 
                 simm$cv_dog1[[3]])), 
           max(c(simm$cv_dog1[[1]], 
                 simm$cv_dog1[[2]], 
                 simm$cv_dog1[[3]])))
for (i in c(1:3)){
...
for (i in c(1:3)){
  if(simm$min_MSE1[i] < simm$min_MSE3[i]){
    if(simm$DoG1[[i]][6] >0){
      data[data$group == levels(simm$group)[i],]$DoG <- dog1(sigma = simm$sigma1[[i]], x = data[data$group == levels(simm$group)[i],]$diffstim)*-1
    }else{
      data[data$group == levels(simm$group)[i],]$DoG <- dog1(sigma = simm$sigma1[[i]], x = data[data$group == levels(simm$group)[i],]$diffstim)
    }
  }else{
        if(simm$DoG3[[i]][6] >0){
      data[data$group == levels(simm$group)[i],]$DoG <- dog3(sigma = simm$sigma3[[i]], x = data[data$group == levels(simm$group)[i],]$diffstim)*-1
    }else{
      data[data$group == levels(simm$group)[i],'DoG'] <- dog3(sigma = simm$sigma3[[i]], x = data[data$group == levels(simm$group)[i],]$diffstim)
    }
  }
}
```
...
if(min_MSE < min_MSE3){
  if (DoG[6] > 0){
    data$previousDoG <- dog1(sigma = sigma, x = abs(data$diffresp))*-1
    }else{
    data$previousDoG <- dog1(sigma = sigma, x = abs(data$diffresp))
    }}else{
      if (DoG3[6] > 0){
      data$previousDoG <- dog3(sigma = sigma, x = abs(data$diffresp))*-1
      }else{
      data$previousDoG <- dog3(sigma = sigma, x = abs(data$diffresp))
      }
    }

modPreviousResp <- lm(fourier_error ~ group * time * previousDoG, data = data)
```
...
serialbiasprevresp <- lapply(gr, function(x) 
  folded_serial_bias(prevcurr = x$diffresp, error = x$fourier_error, 
              window = window, step = step)) %>%
  rbindlist(., idcol='id') %>%
  as.data.frame() %>%
  mutate(session = substr(id, 3, 4), group = substr(id, 1, 1)) %>%
...
if(min_MSE < min_MSE3){
  if (DoG[6] > 0){
    data$futureDoG <- dog1(sigma = sigma, x = abs(data$difffuture))*-1
    }else{
    data$futureDoG <- dog1(sigma = sigma, x = abs(data$difffuture))
    }}else{
      if (DoG3[6] > 0){
      data$futureDoG <- dog3(sigma = sigma, x = abs(data$difffuture))*-1
      }else{
      data$futureDoG <- dog3(sigma = sigma, x = abs(data$difffuture))
      }
    }
...
serialbiasfuture <- lapply(gr, function(x) 
  folded_serial_bias(prevcurr = x$difffuture, error = x$fourier_error, 
              window = window, step = step)) %>%
  rbindlist(., idcol='id') %>%
  as.data.frame() %>%
  mutate(session = substr(id, 3, 4), group = substr(id, 1, 1)) %>%
