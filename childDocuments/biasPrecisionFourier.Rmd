...
```{r, results = 'asis'}
groupedSubjSes <- data %>%
  group_by(session, subject, time, group) %>%
  nest() %>%
...
data %>%
    ...
    group_by(subject) %>%
    summarize("n sessions" = n())
# since some only have 1 session, cannot build some mixed effects models
...
mod2 <- lm(precision ~ group * session, 
           data = groupedSubjSes)
#mod2.2 <- lm(precision ~ group * ordered(session), data = groupedSubjSes)
mod3 <- lme(precision ~ group * session, random = ~1|subject, 
            data = groupedSubjSes[groupedSubjSes$session %in% c('S1', 'S4'),])
mod4 <- lme(precision ~ group * session, random = ~(1+session)|subject,
            data = groupedSubjSes[groupedSubjSes$session %in% c('S1', 'S4'),])
#mod5 <- lme(precision ~ group * session, random = ~(1)|subject,
#            data = groupedSubjSes)
# cannot be build since some subjects just have 1 session 
# (effects of session and subject are confounded)
mod0$sigma # best with continuous
mod1$sigma
summary(mod2)$sigma
mod3$sigma
mod4$sigma # best with categorical
AIC(mod1); AIC(mod2); AIC(mod2.2) AIC(mod3); AIC(mod4)
```
...
summary(glmer(precision ~ group * time + (1|subject), 
  ...
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
        caption = 'Log-linked gamma generalized linear mixed model with data for encephalitis group only. The response is precision and regressors are group, time, its interaction and subject as a random factor.') %>%
    kable_styling(latex_options = "HOLD_position", 
                bootstrap_options = "striped", full_width = F)
...
Anova(glmer(precision ~ group * time + (1|subject), 
            ...
            data = groupedSubjSes[groupedSubjSes$group == 'E',])) %>%
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
        caption = "Anova for the log-linked gamma generalized linear mixed-effects model with data for encephalitis group only. The response is precision and regressors are group, time, its interaction and subject as a random factor.") %>%
  kable_styling(latex_options = "HOLD_position", 
                bootstrap_options = "striped", full_width = F)
```
