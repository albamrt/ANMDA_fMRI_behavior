...
```{r, results = 'asis'}
groupedSubjSes <- data %>%
  group_by(session, subject, time, group, treatment) %>%
  nest() %>%
...
groupedData <- data %>%
  group_by(group, session, time, treatment) %>%
  nest() %>%
  mutate(JV10e = map(.x = data, .f = ~bays_2009_error(X = .x$fourier_R, Tg = .x$S_rad))) %>%
  mutate(JV10 = map(.x = data, .f = ~bays_2009_fit(X = .x$fourier_R, Tg = .x$S_rad, return.ll = TRUE))) %>%
  mutate(precision = map_dbl(JV10e, 1), 
...
```{r fig.cap = 'Bias by group and session.', fig.show='hold', out.width = '49%', fig.ncol = 2}
groupedSubjSes %>%
  ...
  labs(x = label(data$session), y = "Bias", color = label(data$group), 
...
```{r fig.cap = 'Precision by group and session.', fig.show='hold', out.width = '49%', fig.ncol = 2}
groupedSubjSes %>%
  ...
  #                                               color = group), linetype = "dashed") +
    scale_color_manual(values=colors) +
  theme(legend.position=c(.7, .8)) +
  labs(x = label(data$session), y = "Precision", color = label(data$group), 
       group = label(data$group)) 
```
...
mod7 <- glm(precision ~ group * time + group * treatment,
      family = Gamma(link="log"), data = groupedSubjSes)
mod8 <- glm(precision ~ group*time*treatment ,
      family = Gamma(link="log"), data = groupedSubjSes)
mod8.2 <- glm(precision ~ group*session*treatment ,
      family = Gamma(link="log"), 
      data = groupedSubjSes[groupedSubjSes$session %in% c('S1', 'S4'),])
with(groupedSubjSes, table(group,session,treatment))
mod8.3 <- glmer(precision ~ group * time * treatment + (1|subject), 
      family = Gamma(link="log"), 
      data = groupedSubjSes)
# not working:
...
###### Just for the encephalitis group (so we can take into account the 4 sessions)
```{r eval = FALSE}
modE <- glm(precision ~ time*treatment ,
      family = Gamma(link="log"), data = groupedSubjSes[groupedSubjSes$group == 'E',])
modE.2 <- glm(precision ~ session*treatment ,
      family = Gamma(link="log"), 
      data = groupedSubjSes[groupedSubjSes$group == 'E',])
with(groupedSubjSes, table(group,session,treatment))
modE.3 <- glmer(precision ~  time * treatment + (1|subject), 
      family = Gamma(link="log"), 
      data = groupedSubjSes[groupedSubjSes$group == 'E',])
```
...
```{r}
summary(glmer(precision ~  time + (1|subject), 
      family = Gamma(link="log"), 
      data = groupedSubjSes[groupedSubjSes$group == 'E',]))$coefficients %>%
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
        caption = 'Log-linked gamma generalized linear mixed model with data for encephalitis group only. The response is precision and regressors are time and subject as a random factor.') %>%
    kable_styling(latex_options = "HOLD_position", 
                bootstrap_options = "striped", full_width = F)

Anova(glmer(precision ~  time + (1|subject), 
            family = Gamma(link="log"), 
            data = groupedSubjSes[groupedSubjSes$group == 'E',])) %>%
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
        caption = "Anova for the log-linked gamma generalized linear mixed-effects model with data for encephalitis group only. The response is precision and regressors are time and subject as a random factor.") %>%
  kable_styling(latex_options = "HOLD_position", 
                bootstrap_options = "striped", full_width = F)
```
