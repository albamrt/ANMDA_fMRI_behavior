...
g <- data %>%
  ggplot(aes(x = session, y = abs(error), color = group, text = paste(subject))) +
...
data %>%
  ggplot(aes(x = session, y = abs(error), color = group, group = group)) +
...
```{r eval = FALSE}
mod1 <- glmer(abs(error) ~ group*time + (1 + time | subject), 
      family = Gamma(link="log"), data = data)
mod2 <- lmer(log(abs(error)) ~ group*time + (1 + time | subject), data = data) 
plot(mod1)
plot(mod2) # better
```

```{r}
summary(lmer(abs(error) ~ group*time + (1 + time | subject), 
...
```{r eval = FALSE}
mod1 <- glmer(abs(error) ~ group*session + (1 + time | subject), 
      family = Gamma(link="log"), data = data)
mod2 <- lmer(log(abs(error)) ~ group*session + (1 + time | subject), data = data) 
plot(mod1)
plot(mod2) # better
```

```{r}
summary(lmer(abs(error) ~ group*session + (1 + time | subject), 
             data = data))$coefficients %>%
  kable(booktabs = TRUE, escape = FALSE, digits = digits,
...
for(i in seq_along(unique(data$group))){
  data %>%
    ...
    ggplot(aes(x = session, y = abs(error), color = group)) +
