---
title: "Trials with low response times"
author: "Alba Morató"
date: "25/3/2021"
...
Let us start by plotting trials with smaller response times more in detail:
```{r}
hist(data$RT[data$RT<1], border = 'white', xlab = 'Response Time', main = '')
hist(data$RT[data$RT<0.5], border = 'white', xlab = 'Response Time', main = '')
```

Let us check trials with response time < 0.25.

## Trials with response time < 0.25


Let us check how these trials distribute among subjects:
```{r}
low_RT <- data %>% 
...
```{r eval = FALSE}
kable(low_RT, digits = digits, booktabs = TRUE, escape = FALSE,
      caption = 'Proportion of trials with response time < 0.25.',
      col.names = c(label(data$subject), label(data$session), 
                    "Number of trials with RT<0.25", "Total number of trials", 
                    "Prop of trials with RT<0.25")) %>%
...
ggplot(data = low_RT, aes(x = session, y = prop_low_RT, color = group, group = group)) +
...
ggplot(data = low_RT, aes(x = prop_low_RT, fill = group)) +
  geom_histogram(bins = 20) + 
  facet_wrap(vars(group)) +
  scale_fill_manual(values = as.vector(colors))
```

Are there differences in the proportion of trials with RT $< 0.25$ between groups?

```{r}
data0 = data[data$RT<0.25,]
```
...
data %>% 
  group_by(group, session) %>%
  summarise(low_RT = sum(RT<0.25), trials = n(), prop_low_RT = sum(RT<0.25)/n()) %>%
  select(c(session, group, prop_low_RT)) %>%
  spread(session, prop_low_RT) %>%
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
        caption = 'Summary of proportion of trials with RT<0.25 by group and session.',
        align = c('l', rep('c', length(unique(data$session)) + 1)),
                col.names = c(label(data$group), levels(data$session))) %>%
  kable_styling(position = "center",
                bootstrap_options = "striped", full_width = F) %>%
...
```{r echo = TRUE}
mod <- glm(prop_low_RT ~ group, data = low_RT, family = binomial)
summary(mod)

mod2 <- glm(prop_low_RT ~ group*session, data = low_RT, family = binomial)
summary(mod2)
```

It seems the proportion of trials with RT $\approx 0$ is not associated with the `group` they belong to.

```{r eval = FALSE, fig.cap = 'Error distribution by subject and session', fig.height = 40, fig.width = 8}
...
```{r}
data$RT_cat <- factor(data$RT<0.25, levels = c(TRUE, FALSE), labels = c('<0.25', '>0.25'))

groupedSubjSes <- data %>%
  group_by(session, subject, RT_cat) %>%
  nest() %>%
...
groupedData %>%
  select(group, session, RT_cat, precision, bias) %>%
  as.data.frame() %>%
  reshape(idvar = c('group', 'RT_cat'), timevar = 'session', direction = "wide",
          v.names = c('precision', 'bias')) %>%
  select(c("group", 'RT_cat', all_of(aux0))) %>%
  kable(col.names = c(Hmisc::label(data$group), 'RT',
        ...
        digits = digits) %>%
  add_header_above(c(" " = 2, aux)) %>%
  kable_styling(latex_options = "HOLD_position", 
                bootstrap_options = "striped", full_width = F)
```

```{r fig.cap = 'Bias and precision by group and session for RT<0.25.', fig.show='hold', out.width = '49%'}
groupedSubjSes %>%
  filter(RT_cat == '<0.25') %>%
...
### Bias
```{r, fig.align='center', fig.height = 3, fig.cap = 'Bias by group through time'}
groupedData %>%
  ggplot(aes(x = session, y = abs(bias), color = group, group = RT_cat)) +
  geom_line(aes(linetype = RT_cat), show.legend = TRUE) +
  scale_color_manual(values = colors) +
  theme_minimal(base_size = 15) +
  labs(x = Hmisc::label(data$time), y = "Absolute bias", color = 'Group', linetype = 'RT') +
  facet_wrap(~ group) 
```
...
```{r }
tryCatch(
  summary(lme(abs(bias) ~ group * time*RT_cat, random = ~(1+time)|subject, data = groupedSubjSes))$tTable %>%
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
    ...
    kable_styling(latex_options = "HOLD_position", 
                bootstrap_options = "striped", full_width = F),
  error = function(i) {
      summary(lm(abs(bias) ~ group * time*RT_cat, data = groupedSubjSes))$coefficients %>%
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
...
tryCatch(
  anova(lme(abs(bias) ~ group * time*RT_cat, random = ~(1+group)|subject, data = groupedSubjSes)) %>%
    kable(digits = digits, booktabs = TRUE, escape = FALSE,
      ...
      kable_styling(latex_options = "HOLD_position", 
                  bootstrap_options = "striped", full_width = F),
  error = function(i) {
      anova(lm(abs(bias) ~ group * time*RT_cat, data = groupedSubjSes)) %>%
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
...
```{r }
tryCatch(
  summary(lme(abs(bias) ~ group * session*RT_cat, random = ~(1+session)|subject,
  ...
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
    ...
    kable_styling(latex_options = "HOLD_position", 
                bootstrap_options = "striped", full_width = F),
  error = function(i) {
      summary(lm(abs(bias) ~ group * session*RT_cat, 
...
tryCatch(
  anova(lme(abs(bias) ~ group * session*RT_cat, random = ~(1+session)|subject,
            data = groupedSubjSes[groupedSubjSes$session %in% c('S1', 'S4'),])) %>%
    kable(digits = digits, booktabs = TRUE, escape = FALSE,
      ...
      kable_styling(latex_options = "HOLD_position", 
                  bootstrap_options = "striped", full_width = F),
  error = function(i) {
      anova(lm(abs(bias) ~ group * session*RT_cat, 
               data = groupedSubjSes[groupedSubjSes$session %in% c('S1', 'S4'),])) %>%
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
        caption = 'Anova of the linear model where the response is bias and regressors are group, time, its interaction and subject as a random factor..') %>%
    kable_styling(latex_options = "HOLD_position", 
...
```{r, fig.align='center', fig.height = 3, fig.cap = 'Precision by group through time'}
groupedData %>%
  ggplot(aes(x = session, y = precision, color = group, group = RT_cat)) +
  geom_line(aes(linetype = RT_cat), show.legend = TRUE) +
  scale_color_manual(values = colors) +
  theme_minimal(base_size = 15) +
  labs(x = Hmisc::label(data$time), y = "Precision", color = 'Group', linetype = 'RT') +
  facet_wrap(~ group) 
```
...
groupedSubjSes <- groupedSubjSes[!is.infinite(rowSums(groupedSubjSes[,c(5,6)])),]

summary(lme(precision ~ group * time*RT_cat, random = ~(1)|subject, data = groupedSubjSes))$tTable %>%
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
...
anova(lme(precision ~ group * time*RT_cat, random = ~(1)|subject, data = groupedSubjSes)) %>%
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
...
```{r}
summary(lme(precision ~ group * session*RT_cat, random = ~(1)|subject, 
            data = groupedSubjSes[groupedSubjSes$session %in% c('S1', 'S4'),]))$tTable %>%
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
...
anova(lme(precision ~ group * session*RT_cat, random = ~(1)|subject, 
          data = groupedSubjSes[groupedSubjSes$session %in% c('S1', 'S4'),])) %>%
  kable(digits = digits, booktabs = TRUE, escape = FALSE,
