...
######################### PACKAGES #########################
############################################################
#install.packages('quantreg') # answer 'no'
list.of.packages <- c("circular", "dplyr", "ggforce", "knitr", "lme4", 
                      "lmerTest", "Publish", "tidyr", "tidyverse", "Hmisc",
                      "readr", "tibble", "nlme", "reticulate", "data.table",
                      "readxl", "kableExtra", "lme4", "plotly", "remotes",
                      "DHARMa", 'car', 'rsample')
new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
if(length(new.packages)) install.packages(new.packages)
lapply(list.of.packages, require, character.only = TRUE)

if(!'mixturer' %in% installed.packages()[,"Package"]) remotes::install_github('eddjberry/mixturer')
require(mixturer)
...
crossValSigmaFolded <- function(sigma, data, derivative){
  if(derivative == 1){
    data$DoG <- dnorm_deriv1(abs(data$diffstim), sd = sigma)}
  #data$DoG <- dog1(sigma = sigma, x = data$diffstim)}
  else{if(derivative == 3){
    data$DoG <- dnorm_deriv1(abs(data$diffstim), sd = sigma)}}
  #data$DoG <- dog3(sigma = sigma, x = data$diffstim)}}
...
DoG_sign <- function(x) {if(x[6]>0) return(x*-1) else x}
